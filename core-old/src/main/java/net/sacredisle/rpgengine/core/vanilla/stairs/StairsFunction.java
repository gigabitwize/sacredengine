package net.sacredisle.rpgengine.core.vanilla.stairs;
 ...
 */
public class StairsFunction extends VanillaFunctionImpl<PlayerBlockPlaceEvent> {

    public StairsFunction() {
        super(new EventListener<PlayerBlockPlaceEvent>() {
            ...
            public @NotNull Result run(@NotNull PlayerBlockPlaceEvent event) {
                if (VanillaProvider.shouldRun(StairsFunction.class) && VanillaUtil.isStairs(event.getBlock()))
                    MinecraftServer.getBlockManager().registerBlockPlacementRule(new StairsPlacementRule(event.getBlock()));
                return Result.SUCCESS;
            }
        });
    }

    static class StairsPlacementRule extends BlockPlacementRule {
